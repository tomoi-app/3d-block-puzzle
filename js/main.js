...
function init() {
    ...
    scene.fog = new THREE.Fog(0x87CEEB, 20, 60);  // 遠くをぼかす霧
...
function animate(time) {
    ...
    // スコア表示の更新
    document.getElementById('score-display').innerText = `高度: ${maxY}m`;

    // 高度に応じた背景色の変化 (例: 50mで完全に宇宙になる)
    const skyColor = new THREE.Color(0x87CEEB); // 青空
    const spaceColor = new THREE.Color(0x000011); // 宇宙（深い紺〜黒）
    const progress = Math.min(maxY / 50, 1.0); // 0.0 〜 1.0 の割合
    
    // 現在の高度に合わせて色をブレンド
    const currentColor = skyColor.clone().lerp(spaceColor, progress);
    scene.background = currentColor;
    scene.fog.color = currentColor; // フォグの色も合わせる
